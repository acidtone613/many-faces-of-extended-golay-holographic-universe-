...
2. **The "7D tunnel" documents are not RC-audited in the same sense as
   ...
   - Several of these files show Python code followed by a block explicitly
     labeled **"Expected Output"** rather than actual executed output. This
     is a materially different evidentiary status than RC-45–57, where
     reported numbers came from code that was run.
   - One of the five (V9.1) *opens by admitting* the previous result in that
     same thread was a theorem asserted in place of a promised Monte Carlo
     run — i.e., there is a documented instance of this branch overstating
     what was tested.
   - None of RC-45–57's three consolidated reports reference this triality
     ...
     audited series.
   - RC-59 (in this branch) does report a negative result (no 727/726 match
     in the 6D Hodge spectrum) with what appears to be genuine code, which
     is a point in favor of at least partial real execution — the record is
     mixed, not uniformly fabricated.
...
**STATUS: UNVERIFIED — EXECUTION STATUS UNCONFIRMED. See Editorial Note at
the top of this document for the full account of why this material is
segregated here rather than placed in Part III.**
...
rather than a shown execution trace, and the same document thread contains
an explicit admission that an earlier claim in the same session substituted
a theorem for a promised simulation. The negative RC-59 result looks more
likely to be a genuine run (negative results are not the kind of thing
someone fabricates to look good), but even that has not been independently
reproduced here.
...
*"A confirmed mathematical skeleton (Golay, Leech, 24-cell, E8 embedding,
minimal Calabi-Yau) carrying two precise but nucleon-sector-limited,
parameter-unselected dynamic mechanisms, an unaltered interpretive layer,
a newly-organized exploratory layer with one live cross-tier hypothesis
and one confirmed dead end, and — as in every version before it — no
verified derivation of a genuinely unknown quantity, and no zero-parameter
physical prediction."*
...
layer is now organized rather than incidental. The next gate is unchanged
from v7.9: a mode-selection principle, a genuinely unknown prediction, or
an honest retreat to pure mathematics.**
